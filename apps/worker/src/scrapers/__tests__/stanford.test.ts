...
describe("StanfordScraper", () => {
  ...
  it("paginates through multiple pages", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            events: [mockEvent({ id: 1, title: "Page 1 Event" })],
      ...
      .mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            events: [mockEvent({ id: 2, title: "Page 2 Event" })],
    ...
    expect(result.events[0].title).toBe("Page 1 Event");
    expect(result.events[1].title).toBe("Page 2 Event");
  });

  it("handles events with no geo data", async () => {
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([mockEvent({ geo: undefined })])
    );
    const result = await scraper.scrape();
  ...
  it("handles events with no description", async () => {
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([mockEvent({ description_text: undefined, description: undefined })])
    );
    const result = await scraper.scrape();
  ...
  it("handles events with no end time", async () => {
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([
        mockEvent({
  ...
  it("handles events with no filters object", async () => {
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([mockEvent({ filters: undefined })])
    );
    const result = await scraper.scrape();
    expect(result.events[0].eventType).toBeUndefined();
  ...
  it("handles events with no departments", async () => {
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([mockEvent({ departments: undefined })])
    );
    const result = await scraper.scrape();
    expect(result.events[0].department).toBeUndefined();
  });

  it("handles events with no location at all", async () => {
    vi.stubGlobal(
      "fetch",
      mockFetchSuccess([mockEvent({ location_name: undefined, room_number: undefined })])
    );
    const result = await scraper.scrape();
